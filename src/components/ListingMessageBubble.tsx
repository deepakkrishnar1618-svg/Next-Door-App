...
interface ListingMessageReaction {
  id: number;
  listing_message_id: number;
  user_id: string;
  user_name: string;
  emoji: string;
  created_at: string;
...
interface ListingMessageAttachment {
  id: number;
  listing_message_id: number;
  filename: string;
  file_key: string;
...
export interface ListingMessage {
  ...
  created_at: string;
  updated_at: string;
  attachments?: ListingMessageAttachment[];
  reactions?: ListingMessageReaction[];
...
interface ListingMessageBubbleProps {
...
export default function ListingMessageBubble({
  ...
  const handleTouchEnd = () => {
    if (swipeOffset > 60) onReply(message);
    setSwipeOffset(0);
  };
  ...
  const handleDoubleTap = () => {
    ...
    if (timeSinceLastTap < 300 && timeSinceLastTap > 0) {
      if (window.innerWidth < 1024) onSetActive(!isActive);
  ...
  const handleEmojiSelect = async (emoji: string) => {
    try {
      await fetch(`/api/listing-messages/${message.id}/reactions`, {
        method: "POST",
  ...
  const handleEdit = async () => {
    ...
    try {
      const response = await fetch(`/api/listing-messages/${message.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: 'include',
  ...
  const groupedReactions = (message.reactions || []).reduce((acc, reaction) => {
    ...
    return acc;
  }, {} as Record<string, ListingMessageReaction[]>);
    ...
    >
      ...
      <div className={`flex-1 ${isOwnMessage ? "flex flex-col items-end" : ""}`}>
        ...
        <div className={`relative group inline-block ${
          isEditing ? "max-w-[95%] sm:max-w-2xl" : "max-w-[85%] sm:max-w-md"
        }`}>
          {/* Mobile action bar */}
          {showMobileActionBar && (
            <>
              <div className="fixed inset-0 z-20 lg:hidden" onClick={() => onSetActive(false)} />
              <div 
                className={`z-30 lg:hidden ${
                  barPosition === 'fixed-top' || barPosition === 'fixed-bottom' ? 'fixed' : 'absolute'
                  ...
                  <div className="relative">
                    ...
                    {showMenu && (
                      <div className={`absolute right-0 bg-white dark:bg-dark-surface rounded-button-rect shadow-soft dark:shadow-soft-dark border border-slate-200 dark:border-slate-700 py-1 w-32 z-50 ${
                        barPosition === 'below' || barPosition === 'fixed-bottom' ? 'bottom-full mb-1' : 'top-full mt-1'
                      }`}>
                        <button
                          onClick={() => { handleCopy(); setShowMenu(false); onSetActive(false); }}
                        ...
                        {isOwnMessage && canEditOrDelete() && (
                          <>
                            <button
                              onClick={() => { setIsEditing(true); setShowMenu(false); onSetActive(false); }}
                              className="w-full px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-elevated flex items-center gap-2 text-slate-800 dark:text-white"
                            >
                              <Edit2 className="w-3 h-3" /> Edit
                            </button>
                            <button
                              onClick={() => { setShowMenu(false); onSetActive(false); handleDelete(); }}
                        ...
                        {message.id > 0 && onMessageInfo && (
                          <button
                            onClick={() => { setShowMenu(false); onSetActive(false); onMessageInfo(message.id); }}
                            className="w-full px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-elevated flex items-center gap-2 text-slate-800 dark:text-white"
                          >
                            <Info className="w-3 h-3" /> Message Info
                          </button>
                        )}
                      </div>
                    )}
          ...
          >
            {message.reply_to_content && message.reply_to_message_id && (
              ...
              >
                ...
                <div className={`text-xs line-clamp-2 font-outfit ${
            ...
            {isEditing ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={handleEdit} className="text-xs px-3 py-1 bg-white/20 hover:bg-white/30 rounded-button-rect transition-colors font-outfit">Save</button>
                  <button onClick={() => { setIsEditing(false); setEditContent(message.content); }} className="text-xs px-3 py-1 bg-white/20 hover:bg-white/30 rounded-button-rect transition-colors font-outfit">Cancel</button>
                </div>
              </div>
            ) : (
          ...
          <div className={`hidden lg:block absolute top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 ${
            ...
            <div className="flex items-center gap-1 bg-white dark:bg-dark-surface rounded-button shadow-soft dark:shadow-soft-dark border border-slate-200 dark:border-slate-700 px-2 py-1.5">
              <button onClick={() => onReply(message)} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-dark-elevated transition-colors" title="Reply">
                <Reply className="w-4 h-4 text-slate-600 dark:text-slate-400" />
              </button>
              <div className="w-px h-5 bg-slate-200 dark:bg-slate-700"></div>
              {!showQuickReactions ? (
                <button onClick={() => setShowQuickReactions(true)} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-dark-elevated transition-colors" title="Add reaction">
                  <Smile className="w-4 h-4 text-slate-600 dark:text-slate-400" />
                </button>
              ) : (
                <>
                  {['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) => (
                    <button key={emoji} onClick={() => { handleEmojiSelect(emoji); setShowQuickReactions(false); }} className="text-lg hover:scale-125 transition-transform p-1">{emoji}</button>
                  ))}
                </>
              )}
              {((message.id > 0 && onMessageInfo) || (isOwnMessage && canEditOrDelete())) && (
                <>
                  <div className="w-px h-5 bg-slate-200 dark:bg-slate-700"></div>
                  <div className="relative">
                    <button onClick={() => setShowMenu(!showMenu)} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-dark-elevated transition-colors">
                    ...
                    {showMenu && (
                      <div className="absolute top-full mt-1 right-0 bg-white dark:bg-dark-surface rounded-button-rect shadow-soft dark:shadow-soft-dark border border-slate-200 dark:border-slate-700 py-1 w-32 z-10">
                        {isOwnMessage && canEditOrDelete() && (
                          <>
                            <button onClick={() => { setIsEditing(true); setShowMenu(false); }} className="w-full px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-elevated flex items-center gap-2 text-slate-800 dark:text-white font-outfit">
                              <Edit2 className="w-3 h-3" /> Edit
                            </button>
                            <button onClick={() => { setShowMenu(false); handleDelete(); }} className="w-full px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-elevated flex items-center gap-2 text-error font-outfit">
                              <Trash2 className="w-3 h-3" /> Delete
                            </button>
                          </>
                        )}
                        {message.id > 0 && onMessageInfo && (
                          <button onClick={() => { setShowMenu(false); onMessageInfo(message.id); }} className="w-full px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-dark-elevated flex items-center gap-2 text-slate-800 dark:text-white font-outfit">
                            <Info className="w-3 h-3" /> Message Info
                          </button>
                        )}
                      </div>
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {Object.entries(groupedReactions).map(([emoji, reactions]) => (
                <div key={emoji} className="relative">
                  <button
                    onClick={(e) => {
                      if (isMobile) { e.stopPropagation(); setReactionPopupEmoji(reactionPopupEmoji === emoji ? null : emoji); }
                      else { handleReactionClick(emoji); }
                    }}
                    onMouseEnter={() => { if (!isMobile) setReactionPopupEmoji(emoji); }}
                    onMouseLeave={() => { if (!isMobile) setReactionPopupEmoji(null); }}
                  ...
                  {!isMobile && reactionPopupEmoji === emoji && (
                    <ReactionDetailsPopup emoji={emoji} reactions={reactions} isOpen={true} onClose={() => setReactionPopupEmoji(null)} isMobile={isMobile} isOwnMessage={isOwnMessage} />
                  )}
                </div>
              ))}
            </div>
            {isMobile && reactionPopupEmoji && groupedReactions[reactionPopupEmoji] && (
              <ReactionDetailsPopup emoji={reactionPopupEmoji} reactions={groupedReactions[reactionPopupEmoji]} isOpen={true} onClose={() => setReactionPopupEmoji(null)} isMobile={isMobile} isOwnMessage={isOwnMessage} />
            )}
          </>
        )}
        ...
        <div className="text-xs mt-1 flex items-center gap-2 font-outfit">
          ...
          {optimisticStatus === 'failed' && onRetry && onDelete && (
            <div className="inline-flex items-center gap-2">
              ...
              <button onClick={onRetry} className="px-2 py-1 bg-gradient-primary text-white rounded-button text-xs font-semibold hover:scale-105 transition-transform shadow-soft">Retry</button>
              <button onClick={onDelete} className="px-2 py-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-button text-xs font-semibold hover:scale-105 transition-transform">Delete</button>
            </div>
          )}
        </div>
      </div>
